...
describe('The write route should take an url and', () => {
  ...
  it('fetch url from db if it already exists', (done) => {
    const longurl = 'http://givemepotatoes.com/7392';
    const expectedHash = md5(longurl).substring(0, 6);
  ...
  it('handle hash collision if longurl does not match', (done) => {
    ...
    // Sanity check
    expect(md5(longurl1).substring(0, 6))
      .toBe(md5(longurl2).substring(0, 6));
